...
public class FlyMovement : Movement {
  public override IEnumerator Traverse (Tile tile) {
    ...
    float dist = Mathf.Sqrt(Mathf.Pow(tile.position.x - unit.tile.position.x, 2) + Mathf.Pow(tile.position.y - unit.tile.position.y, 2));
    unit.Place(tile);
    ...
    if (Mathf.Abs(toTile.x) > Mathf.Abs(toTile.z))
      directions = toTile.x > 0 ? Directions.East : Directions.West;
    else
      directions = toTile.z > 0 ? Directions.North : Directions.South;

    yield return StartCoroutine(Turn(directions));
